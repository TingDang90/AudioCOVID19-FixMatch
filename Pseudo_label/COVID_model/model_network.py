...
def define_audio_slim(
    ...
    with slim.arg_scope(
        ...
        weights_initializer=tf.truncated_normal_initializer(
            stddev=params.INIT_STDDEV, seed=9
...
def add_student_teacher_graph(FLAGS, reuse_model=False):
    """ create a student and teacher graph,
        reusing the student graph if reuse_model is True """
    with tf.Graph().as_default() as graph:
        add_training_graph(FLAGS, scope_name="student")
        if not reuse_model:
            add_training_graph(FLAGS, scope_name="teacher")
        return graph
        


def add_training_graph(FLAGS, scope_name=""):
    """Define the TensorFlow Graph in the scope_name."""
    with tf.variable_scope(scope_name):
        ...
        with tf.variable_scope("train"):
            ...
            lr1_lab = tf.train.exponential_decay(
                FLAGS["lr1"], global_step, number_decay_steps, base_of, staircase=True, name="train_lr1_lab"
            )
            lr2_lab = tf.train.exponential_decay(
                FLAGS["lr2"], global_step, number_decay_steps, base_of, staircase=True, name="train_lr2_lab"
            )
            lr1_unlab = tf.train.exponential_decay(
                FLAGS["lr1"], global_step, number_decay_steps, base_of, staircase=True, name="train_lr1_unlab"
            )
            lr2_unlab = tf.train.exponential_decay(
                FLAGS["lr2"], global_step, number_decay_steps, base_of, staircase=True, name="train_lr2_unlab"
            )
            ...
            if FLAGS["is_diff"]:  # use different learning rate for vgg and others
                ...
                var1 = find_variables_in_scope(scope_name)[0: FLAGS["trained_layers"]]
                var2 = find_variables_in_scope(scope_name)[FLAGS["trained_layers"]:]

                train_op1_lab = tf.train.AdamOptimizer(learning_rate=lr1_lab, epsilon=params.ADAM_EPSILON).minimize(
                    loss, var_list=var1, global_step=global_step, name="train_op1_lab"
                )
                train_op2_lab = tf.train.AdamOptimizer(learning_rate=lr2_lab, epsilon=params.ADAM_EPSILON).minimize(
                    loss, var_list=var2, global_step=global_step, name="train_op2_lab"
                )  # fixed 'var1'
                train_op_lab = tf.group(train_op1_lab, train_op2_lab, name="train_op_lab")  # noqa: F841

                train_op1_unlab = tf.train.AdamOptimizer(learning_rate=lr1_unlab, epsilon=params.ADAM_EPSILON).minimize(
                    loss, var_list=var1, global_step=global_step, name="train_op1_unlab"
                )
                train_op2_unlab = tf.train.AdamOptimizer(learning_rate=lr2_unlab, epsilon=params.ADAM_EPSILON).minimize(
                    loss, var_list=var2, global_step=global_step, name="train_op2_unlab"
                )  # fixed 'var1'
                train_op_unlab = tf.group(train_op1_unlab, train_op2_unlab, name="train_op_unlab")  # noqa: F841
            else:
                print("is diff is false!!!")
...
class TensorHolder:
    ...
    def __init__(self, sess, vggish_input_tensor_name, model_name=""):
        """
        Parameters
        ----------
        sess : tf.Session
        ...
        """
        self.vgg_tensor = sess.graph.get_tensor_by_name(model_name + vggish_input_tensor_name)
        self.index_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/index:0")
        self.index2_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/index2:0")
        self.dropout_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/dropout_rate:0")
        self.logit_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/Output/prediction:0")
        self.logitsym_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/symptom/prediction_sym:0")
        self.symptom_tensor = sess.graph.get_tensor_by_name(model_name + "train/symptoms:0")
        self.labels_tensor = sess.graph.get_tensor_by_name(model_name + "train/labels:0")
        self.global_step_tensor = sess.graph.get_tensor_by_name(model_name + "train/global_step:0")
        self.lr1_lab_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr1_lab:0")
        self.lr2_lab_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr2_lab:0")
        self.lr1_unlab_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr1_unlab:0")
        self.lr2_unlab_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr2_unlab:0")
        self.loss_tensor = sess.graph.get_tensor_by_name(model_name + "train/loss_op:0")
        self.cla_loss_tensor = sess.graph.get_tensor_by_name(model_name + "train/cla_loss:0")
        self.reg_loss_tensor = sess.graph.get_tensor_by_name(model_name + "train/reg_loss2:0")
        self.sym_loss_tensor = sess.graph.get_tensor_by_name(model_name + "train/sym_loss:0")
        self.train_op_lab = sess.graph.get_operation_by_name(model_name + "train/train_op_lab")
        self.train_op_unlab = sess.graph.get_operation_by_name(model_name + "train/train_op_unlab")
        self.summary_op = tf.summary.merge_all(scope=f"^{model_name}")


    def training_tensors(self, is_labelled):
        ...
        if is_labelled:
            ...
            return [
                self.global_step_tensor,
                self.lr1_unlab_tensor,
                self.lr2_unlab_tensor,
                self.logit_tensor,
                self.logitsym_tensor,
                self.loss_tensor,
                self.summary_op,
