...
def spec_augment(spec: np.ndarray, time_warp_rate=0.99, num_mask=2, freq_masking_max_percentage=0.1, time_masking_max_percentage=0.2):
    ...
    spec = spec.copy()
    print(spec.shape)
    display_spec(spec.copy(), "pre_spec.png")
    spec = tf.Session().run(tf.image.crop_and_resize(np.array([spec]), [[0, 0, 1, time_warp_rate]], box_ind=[0], crop_size=(spec.shape[0], spec.shape[1])))[0].reshape((spec.shape[0] * spec.shape[1], spec.shape[2]))
    print(spec.shape)
    display_spec(spec, "post_resize.png")
    ...
    display_spec(spec, "post_mask.png")
...
def load_data(data_path, is_aug, train_data_portion=1):
...
def load_vad_data(data_path, train_data_portion=1):
    """Load vad data shuffled into train data. updated to reduce train_data_portion"""
    print("start to load data:", data_path)
...
def load_unlabelled_data(data_path):
    ...
    #data.update(data2)  # This simply updates in place...
