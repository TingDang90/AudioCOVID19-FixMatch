...
print("Pseudo-label threshold:", THRESHOLD)
...
def _create_data():
    ...
    train, val, test = util.load_data(data_dir, FLAGS["is_aug"], train_data_portion=FLAGS["train_data_portion"])
...
def main(_):
    ...
    # switch between unlabelled and labelled data each epoch
    switch_unlab_lab = True
    unlab_epoch = True
    ...
    with tf.Session(graph=add_student_teacher_graph(FLAGS, reuse_model=FLAGS["dynamic_label"]), config=sess_config) as sess:
        ...
        for epoch in range(20):
            num_labelled, num_unlabelled = 0, 0  # number of labelled and unlabelled samples per epoch
            if epoch == 0:
                curr_step = 0
            ...
            for sample, label_type in train_data_full:  # generate training batch
                ...
                [num_steps, lr1, lr2, logits, logitsym, loss, summaries, _, clal, regl, syml] = sess.run(
                    tensors_student.training_tensors(is_labelled=label_type == "labelled"),
            ...
            if FLAGS["is_diff"]:
                ...
                print("Labelled LRs:", sess.run(tensors_student.lr1_lab_tensor), sess.run(tensors_student.lr2_lab_tensor))
                print("Unlabelled LRs:", sess.run(tensors_student.lr1_unlab_tensor), sess.run(tensors_student.lr2_unlab_tensor))
                print()
            else:
            ...
            print("Num labelled:", num_labelled, "Num unlabelled:", num_unlabelled)
            print(
            ...
            if FLAGS['shuffle_vad'] and epoch == 14:
                ...
                try:
                    ...
                    print(e)
                train_data_full = util.shuffle_unlabelled(train_data, unlabelled_data, valid_data, data_path=data_dir, train_data_portion=FLAGS["train_data_portion"])
                #checkpoint_path = os.path.join(audio_ckpt_dir + "_final", name_all + params.AUDIO_CHECKPOINT_NAME)
