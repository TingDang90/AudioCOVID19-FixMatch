...
""" edited to scope correctly with model name """
...
def load_vggish_slim_checkpoint(session, checkpoint_path, model_scope_name=""):
    ...
    # Use a Saver to restore just the variables in scope.
    saver = tf.train.Saver({var_name.split(":")[0]: find_variable(model_scope_name + var_name) for var_name in vggish_var_names}, name="vggish_load_pretrained", write_version=1)
