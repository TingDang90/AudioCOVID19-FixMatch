...
def add_student_teacher_graph(FLAGS, reuse_model=False):
    with tf.Graph().as_default() as graph:
        alpha = tf.get_variable("betadist_alpha", dtype=tf.float32, shape=(), initializer=tf.random_uniform_initializer(maxval=1, minval=0),
...
def add_training_graph(FLAGS, scope_name=""):
    """Define the TensorFlow Graph in the scope scope_name."""
    with tf.variable_scope(scope_name):
        ...
        with tf.variable_scope("train"):
            ...
            labelled_indicator = tf.placeholder(tf.bool, name="labelled_indicator")
            unlabelled_loss_weight = FLAGS["unlabelled_loss_weight"]
            ...
            loss = tf.cond(labelled_indicator, lambda: loss, lambda: loss * unlabelled_loss_weight)
            ...
            if FLAGS["is_diff"]:  # use different learning rate for vgg and others
                ...
                var1 = util.find_variables_in_scope(scope_name)[0: FLAGS["trained_layers"]]
                var2 = util.find_variables_in_scope(scope_name)[FLAGS["trained_layers"]:]
...
class TensorHolder:
    ...
    def __init__(self, sess, vggish_input_tensor_name, model_name=""):
        ...
        self.lr1_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr1:0")
        self.lr2_tensor = sess.graph.get_tensor_by_name(model_name + "train/train_lr2:0")
        ...
        self.train_op = sess.graph.get_operation_by_name(model_name + "train/train_op")
        self.summary_op = tf.summary.merge_all(scope=f"^{model_name}")
        self.labelled_indicator = sess.graph.get_tensor_by_name(model_name + "train/labelled_indicator:0")
        self.latent_tensor = sess.graph.get_tensor_by_name(model_name + "mymodel/Output/fully_connected/BiasAdd:0")
    ...
    def prediction_tensors(self):
        """ Returns a the list of Tensors used in a prediction step """
        return [self.logit_tensor, self.loss_tensor, self.latent_tensor]


    def vad_tensors(self):
        """ Returns a the list of Tensors used in a validation step """
        return [self.logit_tensor, self.logitsym_tensor, self.loss_tensor, self.cla_loss_tensor, self.reg_loss_tensor]


    def feed_dict(self, vgg_tensor, index_tensor, index2_tensor, dropout_tensor, symptom_tensor, labels_tensor, labelled_indicator=True):
        ...
        return {
            ...
            self.labelled_indicator: labelled_indicator,  # boolean indicating labelled or unlabelled
        }
