...
pos_sym = "covid" + "sym"
pos_asym = "covid" + "nosym"
neg_sym = "health" + "sym"
neg_asym = "health" + "nosym"
...
def get_android(uid, COVID):
    ...
    for files in fold_path:  # date, no more than 5, check label
        ...
        is_sym = df[(df["uid"] == uid) & (df['cough_path'].str.contains(files))].iloc[0]["categs"]
        #is_sym = "sym" if df[(df["uid"] == uid) & (df["fold"] == "train") & (df["categs"] == pos_sym)] else "asym"
        data.append({"breath": breath, "cough": cough, "voice": voice, "label": COVID, "breath_opensmile": b_opensmile, "cough_opensmile": c_opensmile, "voice_opensmile": v_opensmile, "info": os.path.join(path, folds, files), "sym": is_sym})
    return data
...
def get_web(uid, COVID):  # date
    ...
    is_sym = df[(df["uid"] == uid) & (df['cough_path'].str.contains(uid))].iloc[0]["categs"]
    print(is_sym)
    data.append({"breath": breath, "cough": cough, "voice": voice, "label": COVID, "breath_opensmile": b_opensmile, "cough_opensmile": c_opensmile, "voice_opensmile": v_opensmile, "info": os.path.join(path, folds, uid), "sym": is_sym})
    return data
