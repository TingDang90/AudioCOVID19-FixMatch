...
##########################################
path = "../data/0426_EN_used_task2/"


def extract_opensmile(sample):
    sample = sample.replace("\\", os.path.sep)

    # make directories
    feature_dir = "OpenSmile_features"
    uid_dir = os.path.join(feature_dir, sample.split(os.path.sep)[0])
    ts_dir = os.path.join(uid_dir, sample.split(os.path.sep)[1])
    for dir_name in [feature_dir, uid_dir, ts_dir]:
